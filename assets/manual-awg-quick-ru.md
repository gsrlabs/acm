...
![](https://github.com/gsr/acm/raw/main/assets/screenshot_1.png)
...
### 3. Команды для запуска (шпаргалка)
...
- После любого изменения конфига (AllowedIPs, I1 и т.д.) — делай `down + up`.
- Если хочешь использовать **раздельное туннелирование** — меняй только строку `AllowedIPs`.
```bash
AllowedIPs = 104.18.0.0/16, 172.67.0.0/16   # реальные подсети OpenAI
```
